...
app.get('/set_cookie', function(request,response) {
    ...
    var my_name = 'Robi';
        response.send('your_name', my_name).send('cookie set'); //Sets name = my_name

    });
app.get('/use_cookie', function(request,response) {
        console.log('In GET /use_cookie', request.cookies);
        var the_name = request.cookies["you_name"];
    ...
    if (typeof userdata[login_username] != 'undefined') {
        ...
        if (user_info["password"] != request.body["password"]) {
           ...
           session.last_login_time = theDate;
            response.cookie(`${login_username} is logged in with data ${JSON.stringify(request.query)} on ${theDate}`);
            return;
        }
